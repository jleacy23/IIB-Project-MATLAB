function build_eq_clk_combined_cd_fd_godard_adaptive_fxp_old_mex(P, cfg)
%BUILD_EQ_CLK_COMBINED_CD_FD_GODARD_ADAPTIVE_FXP_OLD_MEX
%   Compile eq_clk.combined_cd_fd_godard_adaptive_fxp_old to MEX.
%
%   build_eq_clk_combined_cd_fd_godard_adaptive_fxp_old_mex(P, cfg)
%
%   ARCHIVED circular-wrap variant of the combined CD-FD + Godard + CMA
%   block (see combined_cd_fd_godard_adaptive_fxp_old).  Identical call
%   signature to the streaming version, so this script mirrors
%   build_eq_clk_combined_cd_fd_godard_adaptive_fxp_mex exactly, only the
%   target function and MEX output name differ.
%
%   Inputs
...
%     P.CR_ki, P.CR_kp                      (Godard PI loop gains)
...
%     P.FxpConfig_CombGodard                - composite fxp config: string,
...
%     P.AEQ_Mode, P.AEQ_BlockLen, P.AEQ_SubframeBlocks, P.AEQ_RScale
    ...
    mexFile = fullfile(srcDir, '+eq_clk', ...
        'combined_cd_fd_godard_adaptive_fxp_old_mex.mexw64');
    if isfile(mexFile)
        delete(mexFile);
        fprintf('  Deleted existing MEX file: %s\n', mexFile);
    end

    T = eq_clk.combined_cd_fd_godard_adaptive_fxp_types(fxp);
    ...
    args = { ...
        ...
        double(P.CR_ki), ...               % ki  (Godard)
        double(P.CR_kp), ...               % kp  (Godard)
        double(P.Ns), ...                  % NSymb
    ...
    codegen('-config', cfg, ...
            ...
            '-o', fullfile(srcDir, '+eq_clk', ...
                'combined_cd_fd_godard_adaptive_fxp_old_mex'));
    fprintf('  eq_clk.combined_cd_fd_godard_adaptive_fxp_old_mex  OK\n');
end
