...
\paragraph{Pilots}
Alternatively the update can be driven directly from the pilot symbol,
\begin{equation}
    \Delta \mathbf{h} = \mu e^* \mathbf{x}_p = \mu (\hat{s} - s)^* \mathbf{x}_p,
\end{equation}
where $s$ and $\hat{s}$ are the transmitted and received pilot symbols at the start of each block. With only one pilot symbol per block the error is formed once and the weights updated once, so neither scales with $P$, as summarised in \cref{tab:adaptive_cost_pilot}.
\begin{table}[H]
    \centering
    \begin{tabular}{lcc}
        \toprule
