function rx = qam_matched_filter(y, sps, pulse, rolloff, span)
%QAM_MATCHED_FILTER  Apply a matched filter to a received signal.
%
%   rx = qam_matched_filter(y, sps, pulse)
%   rx = qam_matched_filter(y, sps, pulse, rolloff, span)
%
%   Inputs
...
%     rx      - filtered signal [samples x NPol], same size as y
    ...
    elseif strcmp(pulse, 'rrc')
        if nargin < 5
            error('qam_matched_filter:missingArgs', ...
        ...
        error('qam_matched_filter:badPulse', ...
