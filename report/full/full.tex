...
\begin{equation}
    ...
    \label{eq:cd_tf}
\end{equation}
whose inverse Fourier transform gives a chirp impulse response
\begin{equation}
...
The equaliser must span the channel memory introduced by dispersion so the number of taps required is at least
\begin{equation}
    N_\mathrm{DS} = \lceil\frac{DL\Delta\lambda}{T}\rceil = \lceil\frac{2\pi|\beta_2|L\Delta f}{T}\rceil.
    \label{eq:cd_len}
\end{equation}
where \(T\) is the receiver sampling period. RRC pulse shaping will limit the signal bandwidth such that $\Delta f \approx R_s$. The filter length needed to remove all inter-symbol interference (ISI) from chromatic dispersion is longer than in \cref{eq:cd_len} to account for the effects of pulse shaping \cite{Rozental;15}. An experimentally verified value for several pulse shapes is given by \cite{Ezra;07}
\begin{equation}
    N_\mathrm{CD} = \lceil\frac{6.67|\beta_2|LR_s}{T}\rceil.
\end{equation}
Thus the filter lengths for the 20 km and 80 km networks with an oversampling rate of 2 are 6 and 22 respectively.
