function Out = equalize_fxp(In, D, L, CLambda, Rs, NPol, SpSIn, NFFT, po2Twiddle, T) %#codegen
%EQUALIZE_FXP  Fixed-point overlap-save frequency-domain CD compensation.
%
%   Out = equalize_fxp(In, D, L, CLambda, Rs, NPol, SpSIn, NFFT,
%                            po2Twiddle, T)
%
...
%                  equalize_fxp_types.  If omitted, uses 'fixed32'.
%
...
%       and sum is truncated to the same WL/FL — no bit growth.
    ...
    %% Input extension (same logic as equalize)
    NIn    = size(In, 1);
    AuxLen = NIn / stepLen;
