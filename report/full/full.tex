...
\begin{table}[H]
    ...
    \caption{Fixed-point performance and energy of the combined clock recovery and equalisation implementations at low- ($8/4/6$) and high-precision ($10/6/8$) operating points, with and without $2^k$ twiddle factors. The three precisions are those of the static equaliser, adaptive equaliser and clock recovery stages. The Gardner low-precision case with $2^k$ twiddle factors fails to reach the FEC limit at any SNR. Proposed candidates for an energy-efficient receiver are highlighted in bold.}
...
\end{equation}
where $\mathrm{SNDR_{dB}}$ is the total SNR from quantisation and distortions in decibels. The energy per sample for a range of ADCs is shown in \cref{fig:adc}. A floor of $\approx 1$ pJ/sample is observed for an SNDR lower than 60 dB, corresponding to an ENOB of 9.6. This is higher than the precisions used so far in the DSP blocks so we assume that 1 pJ/sample is the flat energy cost of the ADC. One sample carries the I/Q part of a QPSK symbol which in turn carries 2 bits of information, hence the the ADC consumes 1 pJ/bit.
\begin{figure}[H]
    \centering
...
The 4 candidate implementations for an energy-efficient receiver come from \cref{tab:combined_eq_clk_no_cfo,tab:cr_cost}. The minimum precision for equalisation/clock recovery is an 8 bit static equaliser, 6 bit timing error estimate, and 4 bit adaptive equaliser. The minimum carrier recovery precision is 10 bits for frequency recovery and 4 bits for phase recovery. A higher precision implementation is also given to mitigate the effects of combined quantisation noise across the full pipeline. 
