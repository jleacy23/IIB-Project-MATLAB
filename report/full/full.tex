...
\paragraph{Adaptive equaliser.} \Cref{fig:aeq_fxp} shows how the precision of the adaptive equaliser impacts performance for each implementation while the static equaliser and clock recovery operate at high precision. Without $2^k$ twiddle factors, the performance of Godard and Gardner does not change down to 4 bits of precision, with a large SNR penalty at 2 bits. When $2^k$ twiddle factors are used, 6 bits is optimal: performance is maintained down to 6 bits, a penalty appears at 4 bits, and at 2 bits the quantisation error prevents the FEC limit from being reached at any SNR.

\paragraph{Static equaliser.} \Cref{fig:seq_fxp} shows the same precision sweep for the precision of the FFT/IFFT and the static filter when clock recovery and the adaptive equaliser are held at high precision. The precision requirement is slightly higher here but Godard and Gardner perform well down to 6 bits of precision. This is a notable result when considering the FFT implementation: to avoid overflow each stage of the radix-2 FFT applies a 1/2 scaling with a bit-shift. For an FFT of size 128 this is 7 bit-shifts which at a precision of only 6 bits means all least significant bits at each stage are discarded. This can introduce a large quantisation error, but this does not manifest until 4 bits of precision, where the FEC limit is no longer reachable.
...
\begin{figure}[H]
    ...
    \label{fig:aeq_fxp}
\end{figure}
\paragraph{Clock recovery.} \Cref{fig:clk_fxp} shows the sweep for the precision of the timing error estimate and correction (Farrow interpolation or phase ramp). The loop filter is kept at high precision, as are the static and adaptive equalisers. Without the $2^k$ twiddle factors, Godard exhibits slightly better performance than Gardner, showing no change in performance down to 4 bits of precision. It also clearly outperforms Gardner when $2^k$ twiddle factors are used, as in the floating-point case. The better performance of Godard at low precision can be explained by the effects of quantisation on the estimation and correction stages: forming the Godard estimate with \cref{eq:godard_sum} requires a summation of $\approx 8$ products when $\eta=2, \beta=0.25, N = 128$, which is more prone to quantisation error than the simpler Gardner estimate of \cref{eq:gardner_ted}. However, correction is much simpler for Godard, as it requires only a CORDIC rotation per symbol at the swept precision, whereas Gardner requires further Farrow interpolation, over which quantisation error can accumulate.
\begin{figure}[H]
    \centering
...
\paragraph{Combined performance comparison.} The full carrier recovery chain is then characterised over the same isolated channel, now with a fixed 3\,GHz carrier frequency offset and Wiener phase noise at the 1\,MHz linewidth limit of \cref{sec:cpon_tolerances}, so the result depends only on frequency and phase recovery. Each frequency recovery algorithm is paired with each phase recovery algorithm, and for every pair the pre-FEC BER is estimated by Monte Carlo simulation over 5 trials at each SNR from 0 to 30\,dB. The RSNR --- the SNR at which the pre-FEC BER crosses the FEC limit --- is found by interpolation between adjacent SNR samples, computed per trial and reported as a mean with standard deviation. Frequency recovery is evaluated in both data-aided and blind operation, with the fixed-point precision and the blind observation length \(D\) swept to map their effect on the RSNR.

\paragraph{Fixed-point.} The fixed-point sweep for carrier recovery applies the swept precision to every operation, where precision refers to the number of bits used to represent fractional values. Angles require 3 integer bits to span the range $[-\pi, \pi]$; these integer bits are included in the energy contribution, on top of the precision bits, for any additions or multiplications performed on them. Any CORDIC operations are assumed to be equivalent to a real multiplication at the swept precision as discussed in \cref{chapter:energy}. The Viterbi \& Viterbi filter values of \cref{eq:viterbi_viterbi} are normalised to keep the real and imaginary parts of the summation approximately in the range [-1,1] so only the precision bits contribute to the energy estimate. Rife \& Boorstyn uses a radix-2 FFT which is scaled by 1/2 at each stage, requiring $\log_2(N_{FFT}) = 9$ extra bits of precision which are included in the energy estimate. The weights of the Kay estimator also lie well within the range [-1,1] so only the precision bits must be considered in the energy estimate.
...
The choice of fixed-point precision used in phase recovery will balance receiver energy and performance. \Cref{fig:CR_precision} compares the performance of both algorithms, which is almost identical down to 6 bits, where the RSNR of Viterbi \& Viterbi rises quickly. The additional operations needed by Viterbi \& Viterbi to estimate the phase error mean that quantisation errors compound, degrading performance at low precision. The ability of the pilots-only estimate to match or improve on the RSNR of Viterbi \& Viterbi is likely due to the inherent robustness of QPSK to additive and phase noise; pilots-only will produce a slightly less accurate estimate but as long as it brings the symbol within the correct QPSK quadrant, decoding will be successful.

It is then clear from \cref{fig:CR_precision} that a receiver can save energy at no performance cost by using the pilots-only estimate with 4 bits of precision. The increase in transmitter energy from the slight increase in RSNR at 2 bits of precision may be outweighed by the receiver energy savings, but this will depend on whether the carrier recovery block limits the receiver SNR and on other system parameters such as the wall-plug efficiency of the laser. All further investigations in this section use pilots-only for phase recovery.
\begin{figure}[H]
    \centering
...
The decrease in threshold SNR for longer observation lengths suggests that the performance of the Rife \& Boorstyn algorithm will improve with the blind observation length \(D\), the number of symbols over which the estimate is formed in blind operation (the blind counterpart of the data-aided observation length \(L = 11\)). This is not the case for the differential phase and Kay estimator, where increasing blind observation length has no effect on threshold SNR. The effect of observation length on the RSNR is shown for both algorithms in \cref{fig:observation_length}, where blind Rife \& Boorstyn shows a clear increase in performance over the blind differential phase and Kay estimator for longer observations and improves on data-aided Rife \& Boorstyn after \(\approx 50\) symbols. The differential phase and Kay estimator in blind operation is thus not viable for frequency recovery, but blind Rife \& Boorstyn can provide the best performance at a higher computational cost.
\begin{figure}[H]
    \centering\includegraphics[width=0.65\textwidth]{blind_data.png}
    \caption{RSNR vs blind observation length for the Rife \& Boorstyn and differential phase and Kay estimators.}
    \label{fig:observation_length}
\end{figure}

\subsection{Frequency Recovery Performance}
The fixed-point performance of each frequency recovery algorithm is compared in \cref{fig:FR_precision_performance}, and the corresponding energy savings when switching to the differential phase and Kay estimator are shown in \cref{fig:FR_precision_energy}. Data-aided Rife \& Boorstyn appears to offer no performance improvement on differential phase and Kay but consumes more energy. Blind Rife \& Boorstyn consumes the most energy but offers a significant performance increase over both data-aided algorithms. The performance of all algorithms does not degrade until 4 bits of precision. Decreasing the precision further will again depend on the balance between transmitter and receiver energy. The most energy-efficient algorithm depends on how frequently the CFO must be estimated; frequent estimates will likely favour the differential phase and Kay estimator but in the extreme case of only one CFO estimate per transmission, amortisation makes the energy consumption negligible and blind Rife \& Boorstyn will be preferred. Only using 11 symbols in the CFO estimate for data-aided operation introduces high sensitivity to the noise present in those symbols, hence a high variance in the RSNR is observed. This has significant design implications, as a transmitter tailored to provide the average RSNR at the receiver could cause decoding errors if the noise on the 11 training symbols is particularly high. A safe design would then ensure the receiver SNR is closer to the maximum measured RSNR. The variance could be reduced by averaging the CFO estimate over several subframes, but this requires the CFO to stay constant for long enough and consumes more energy at the receiver.
\begin{figure}[H]
    \centering
...
Blind Rife \& Boorstyn and differential phase and Kay with pilots-only phase recovery are the two most promising approaches for energy-efficient carrier recovery. The energy breakdowns for each are shown in \cref{fig:cr_energy_breakdown}, and the combined energies and RSNRs are listed in \cref{tab:cr_energy_breakdown}. Estimating the phase error with pilots-only consumes very little energy, and blind R\&B dominates energy consumption in estimating CFO even with amortisation over a subframe. Differential phase and Kay with 4 bits of precision offers a lower RSNR for less energy than blind Rife \& Boorstyn with 2 bits of precision, so the most energy-efficient implementation will select one of the other three combinations depending on the balance between transmitter and receiver energy.
\begin{figure}[H]
    \centering
...
\begin{table}[H]
    ...
    \begin{tabular}{lcrrrrc}
        \toprule
        \multirow{2}{*}{FR algorithm} & Fractional & $E_\mathrm{FR}$ & $E_\mathrm{CR}$ & $E_\mathrm{apply}$ & $E_\mathrm{total}$ & RSNR \\
    ...
    \caption{Combined energy and RSNR of the differential phase and Kay estimator and blind Rife \& Boorstyn, both using pilots-only phase recovery.}
