...
The Godard estimate can be used in a feedforward architecture that applies the correction directly in the frequency domain as
\begin{equation}
    Y[k] = X[k]\exp\left(-j\frac{2\pi k_\mathrm{idx}\hat{\tau}_s}{N}\right),
\end{equation}
where $k_\mathrm{idx}$ is the signed bin index
\begin{equation}
    k_\text{idx} \;=\; \begin{cases} k & 0 \le k < N/2 \\ k - N & N/2 \le k < N \end{cases}.
\end{equation}
This is equivalent to a circular shift by $\hat{\tau}_s$ in the time-domain so overlap-save must be used with the overlap size greater than $\hat{\tau}_s$. To track $\hat{\tau}_s$ over multiple blocks a phase unwrapper must be used but this causes the error to grow unbounded, meaning the overlap size will eventually be too large for the FFT. A better approach is to correct errors that are a fraction of a sample in the time domain and resample the input to the FFT when the error reaches a full sample.

\paragraph{Computational complexity} Estimating and correcting the timing error in the frequency domain is highly efficient as it can be performed on the output of the overlap-save static equaliser before the IFFT. Thus the FFT and IFFT are free and the only increase in complexity comes from the estimate and the phase ramp correction with CORDIC. Sharing the same FFT/IFFT means that the timing recovery must use the same overlap size as the static equaliser $N_\mathrm{CD}$. The operation counts per symbol are summarised in
\begin{table}[H]
    \centering
    \begin{tabular}{lcc}
        \toprule
        Step & RM & RA \\
        \midrule
        Estimation & $\frac{4N(1+\beta)/\eta + 2}{N-N_\mathrm{CD}+1}$ & $\frac{4N(1+\beta)/\eta -2}{N-N_\mathrm{CD}+1}$ \\
        \\
        Correction & $\frac{2N}{N-N_\mathrm{CD}+1}$ & 0 \\
        \bottomrule
    \end{tabular}
\end{table}
