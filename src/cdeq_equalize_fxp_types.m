function T = cdeq_equalize_fxp_types(dt) %#codegen
%CDEQ_EQUALIZE_FXP_TYPES  Data-type table for cdeq_equalize_fxp.
%
%   T = cdeq_equalize_fxp_types(dt)
%
%   Returns a struct of fi prototype objects that define every fixed-point
%   type used inside cdeq_equalize_fxp.
%
...
%     T.tw   - FFT twiddle-factor (passed to fft_fxp)
%     T.hcd  - CD frequency-response coefficients
%     T.acc  - accumulator (FFT butterfly + freq-domain multiply)
%
...
%   uniform fixed-point datapath (FPGA / ASIC).
    ...
    switch dt
        ...
        otherwise
            error('cdeq_equalize_fxp_types:BadType', ...
