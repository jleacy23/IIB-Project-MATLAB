function y = adeq_equalize_fxp(x, SpS, Eq, NTaps, Mu, SingleSpike, N1, N2, NOut, T) %#codegen
%ADEQ_EQUALIZE_FXP  Fixed-point adaptive butterfly equalization (CMA / RDE / CMA+RDE).
%
%   y = adeq_equalize_fxp(x, SpS, Eq, NTaps, Mu, SingleSpike, N1, N2, NOut, T)
%
%   Inputs
...
%                     adeq_equalize_fxp_types.  If omitted, calls
%                     adeq_equalize_fxp_types('fixed16').
%
...
%       indexing into a tap-delay line.
    ...
    if nargin < 10 || isempty(T)
        T = adeq_equalize_fxp_types('fixed16');
    end
