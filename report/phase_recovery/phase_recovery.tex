...
\begin{figure}[H]
    ...
    \caption{Viterbi \& Viterbi algorithm block diagram \cite{mello2021digital}}
...
The Blind Phase Search algorithm is an alternative feed-forward algorithm that can be implemented without multipliers. Figure \ref{blind_phase_search} shows its operation. Producing the test rotations \(\theta_\mathrm{b}\) can be done very efficiently with CORDIC by reusing intermediate values, resulting in only \(\sum_{b=1}^{\log_2B} 2^{b+1} = 4B - 4\) shift and add operations for \(B\) rotations \cite{Pfau2009FeedforwardCarrierRecovery}. For a QAM constellation \(\mathcal{S}\), the decision circuit is implemented with \(2(\sqrt{|\mathcal{S}|} - 1)\) comparators. Forming the squared distances \(|d_b[k]|^2\) appears to require a multiplier but the low degree of precision necessary means a look-up table (LUT) can be used \cite{Pfau2009FeedforwardCarrierRecovery}. Additive noise is filtered from the estimate by summing \(L = 2N + 1\) squared distances. Finding the minimum \(m_b[k]\) can be done with \(B - 1\) comparators.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{blind_phase_search.png}
    \caption{Blind Phase Search algorithm block diagram \cite{mello2021digital}}
...
\section{Phase Estimation Frequency}
Both algorithms above produce a phase noise estimate for every symbol. The phase noise is slowly varying so only estimating every \(K\) symbols and applying the same correction to the intermediate symbols may be sufficient. This would reduce the cost of both algorithms by a factor of \(K\) at the cost of increased phase noise variance. Figure \ref{fig:phase_estimation_frequency} shows the effect of \(K\) on the phase noise variance for both algorithms. Both algorithms use pilots to avoid cycle slips \cite{Cheng2013PilotCycleSlip} and exhibit almost identical performance.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{BER vs StepSize.png}
    \caption{Phase noise variance vs phase estimation frequency \(K\)}
    \label{fig:phase_estimation_frequency}
\end{figure}
\section{Filter Length}
Blind Phase Search averages over several minimum distances to filter out noise. It can be seen from Table \ref{tab:blind_phase_search_cost} that the cost of this filter is linear in \(L\) so it should be made as small as possible while still giving acceptable performance. Figure \ref{fig:filter_length} shows the effect of \(L\) on the phase noise variance.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{BER vs Filter.png}
    \caption{Phase noise variance vs filter length \(L\)}
