...
The receiver is evaluated with fixed-point arithmetic, as used in ASIC designs. This captures the effect of quantisation error on each implementation's performance, and underpins a bit-accurate energy model that goes beyond algorithmic complexity. All fixed-point modelling uses the MATLAB Fixed-Point Designer toolkit. To account for pilots at $\pm3\pm3j$ and any small increase in magnitude from channel impairments, the signal is allocated 3 integer bits to give a range of $[-4, 4 - 2^{-m}]$ for the real and imaginary parts in 2's complement, where $m$ is the number of fractional bits determining precision. These integer bits are ignored in energy comparisons between different precisions: they are unaffected by the number of fractional bits, and are often unused since the data symbols sit at $\pm1\pm j$. Real and imaginary parts of equaliser weights are normalised to [-1,1].
...
The loop filter used in both clock recovery algorithms requires care when implementing in fixed point arithmetic. The proportional and integral constants $k_p, k_i$ can be very small so the loop filter output will be quantised to 0 if it shares the same dynamic range as the error estimate. This can be corrected by choosing $k_p, k_i$ to be powers of 2 such that the quantised error terms can be bit-shifted then combined to give the loop-filter output $W[n] = k_pe[n] + k_i\sum_{l=0}^{n-1}e[n-l]$. This requires $W[n]$ to be a high precision accumulator, but the energy consumption from bits changing in the accumulator is determined by the precision of the error terms. To perform energy-efficient timing correction, $W[n]$ is quantised to low precision before being used either in the phase ramp for Godard or Farrow interpolator for Gardner where energy consumption must be minimised. The FFT scales the input by $1/N$ so Godard can end up estimating the timing error from small values. This can be remedied by applying a left bit-shift that approximates multiplying by $\sqrt{N}$, effectively making the forward FFT have unit gain, before quantisation at the precision of the timing error estimate. Then a right bit-shift is applied to undo this before being scaled by $k_i$ and $k_p$ in the loop filter. 
...
The fixed-point implementation then computes the SNR at the FEC limit over a range of precisions. The specified precision is applied to both equalisers, and the error estimation and correction parts of clock recovery. The loop filter still retains a high precision. The energy consumption of each implementation is then computed from the operation counts for each algorithm and the receiver energy model from \cref{chapter:energy}.
...
\end{equation}
with bin spacing $\eta R_s / N$. The estimate is then applied as a continuous-valued time-domain phasor $\exp(-j 2\pi \hat{k}n / N)$ to later samples before they reach the FFT. Estimating the CFO only requires an average over a small number of blocks and applying the phasor only requires one application of CORDIC so the energy consumption of the CFO correction is negligible compared to equalisation and clock recovery. The estimator is placed before the matched filter, whose pass-band is centred at DC and would otherwise bias the centroid toward zero. \Cref{fig:cfo_corrected_float} shows the SNR at the FEC limit for each implementation. The coarse estimate removes enough CFO for all implementations to achieve the FEC limit at the 3 GHz tolerance. Godard and Gardner with the standard FFT/IFFT exhibit the same performance but Godard performs better once the $2^k$ twiddle factors are included. Rounding the twiddle factors impacts both the FFT and IFFT; as Godard operates on data after the FFT it is not affected by the additional loss in accuracy of the IFFT like Gardner.
\begin{figure}[H]
    \centering
...
\paragraph{Adaptive equaliser.} \Cref{fig:aeq_fxp} shows how the precision of the adaptive equaliser impacts performance for each implementation while the static equaliser and clock recovery operate at high precision. Without $2^k$ twiddle factors performance of Godard and Gardner doesn't change until 4 bits of precision, with a large SNR penalty at 2 bits. 6 bits of precision seems optimal when $2^k$ twiddle factors are used before an SNR penalty at 4 bits, and at 2 bits the quantisation error is enough to stop the FEC limit being reached at any SNR.

\paragraph{Static equaliser.} \Cref{fig:seq_fxp} shows the same precision sweep for the precision of the FFT/IFFT and the static filter when clock recovery and the adaptive equaliser are held at high precision. The precision requirement is slightly higher here but Godard and Gardner perform well down to 6 bits of precision. This is a notable result when considering the FFT implementation: to avoid overflow each stage of the radix-2 FFT applies a 1/2 scaling with a bit-shift. For an FFT of size 128 this is 7 bit-shifts which at a precision of only 6 bits means all least significant bits at each stage are discarded. This can introduce a large quantisation error but this does not seem to manifest until 4 bits where the FEC limit is no longer reachable. 
...
\begin{figure}[H]
    ...
    \label{fig:seq_fxp}
\end{figure}
\paragraph{Clock recovery.} \Cref{fig:clk_fxp} shows the sweep for the precision of the timing error estimate and correction (Farrow interpolation or phase ramp). The loop filter is kept at high precision as are the static and adaptive equalisers. Godard exhibits slightly better performance than Gardner without the $2^k$ twiddle factors, showing no change in performance down to 4 bits of precision. It is also clearly outperforms Gardner when $2^k$ twiddle factors are used as in the floating point case. The better performance of Godard at low precision can be explained by considering the effects of quantisation on the estimation and correction portions: forming the Godard estimate with \cref{eq:godard_sum} requires a summation of $\approx 8$ products when $\eta=2, \beta=0.25, N = 128$ which is more prone to quantisation error than the simpler Gardner estimate of \cref{eq:gardner_ted}. However, correction is much simpler for Godard as it just requires a CORDIC rotation per symbol at the swept precision whereas Gardner requires further Farrow interpolation which requires many more operations for quantisation error to accumulate over.
\begin{figure}[H]
    \centering
...
\paragraph{Lowest precision operating point.}
...
The full carrier recovery chain is then characterised over the same isolated channel, now with a fixed 3\,GHz carrier frequency offset and Wiener phase noise at the 1\,MHz linewidth limit of \cref{sec:cpon_tolerances}, so the result depends only on frequency and phase recovery. Each frequency recovery algorithm is paired with each phase recovery algorithm, and for every pair the pre-FEC BER is estimated by Monte Carlo simulation at each SNR from 0 to 30\,dB. The SNR at the FEC limit is found by interpolation between adjacent SNR samples. It is computed per trial and reported as a mean with standard deviation.

\subsection{Fixed Point}
The fixed point sweep for carrier recovery applies the swept precision to every operation, this refers to the number of bits used to represent fractional values. Angles require 3 integer bits to span the range $[-\pi, \pi]$ which are included in the energy contribution on top of the precision bits for any additions or multiplications performed on them. Any CORDIC operations are assumed to be equivalent to a real multiplication at the swept precision as discussed in \cref{chapter:energy}. The Viterbi \& Viterbi filter values of \cref{eq:viterbi_viterbi} are normalised to keep the real and imaginary parts of the summation approximately in the range [-1,1] so only the precision bits contribute to the energy estimate. Rife \& Boorstyn uses a radix-2 FFT which is scaled by 1/2 at each stage, requiring $\log_2(N_{FFT}) = 9$ extra bits of precision which are included in the energy estimate. The weights of the Kay estimator also lie well within the range [-1,1] so only the precision bits must be considered in the energy estimate.
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Frequency recovery as fixed-point precision varies, comparing data-aided differential phase and Kay, data-aided Rife \& Boorstyn, and blind Rife \& Boorstyn, with the corresponding energy savings when switching to differential phase and Kay. Frequency recovery energy amortised over one subframe (3712 symbols).}
